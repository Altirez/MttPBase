...
public class NewSearchActivity extends ConnectionActivity {
    ...
    private ArrayList<TripleTableField> loadSettings() {
        ...
        //if (jsonS == "") {
            ApplicationParameters.chosenSearchFields = new ArrayList<TripleTableField>();
        try {
            if (ApplicationParameters.lastSearchString != null) {
                JSONArray filters = ApplicationParameters.lastSearchString.getJSONArray("filters");
                for (int i = 0; i < filters.length(); i++) {
                    TripleTableField tr = ApplicationParameters.getTableFieldByKey(filters.getJSONObject(i).getString("field_name"), ApplicationParameters.tableFields);
                    if (tr != null)
                        ApplicationParameters.chosenSearchFields.add(tr);
                }
            }
        ...
        //} else {
        //    ApplicationParameters.chosenSearchFields = gson.fromJson(jsonS, type);
        ...
        protected void onPostExecute(JSONObject res) {
            ...
            if (res.has("error")) {
                ...
                try {
                    ...
                    len = arr.length();
                    if (len > 0) {
                        if (arr.getJSONObject(0).has("ID_STATUS_EX2")) {
                            for (int i = 0; i < len; i++) {
                                try {
                                    String utf8String= new String(arr.getJSONObject(i).getString("ID_STATUS_EX2").getBytes("cp1251"));
                                    arr.getJSONObject(i).put("ID_STATUS_EX2", utf8String);
    ...
    public void refreshRequest() {
        final EditText input = new EditText(NewSearchActivity.this);
        if (bookmarkID < 0) {
                            ...
                            new AsyncTask<Pair<String, HashMap<String, String>>, Void, JSONObject>() {
                                JSONObject checkServerCredentials2() throws JSONException {
                                    ...
                                    JSONObject obj = parser.getJSONFromUrl("http://" + ApplicationParameters.MAIN_DOMAIN + "/add_bookmark", new HashMap<String, String>() {{
                                        ...
                                        ArrayList<TripleTableField> p1 = ApplicationParameters.chosenSearchResultsTableFields.size() > 0 ? ApplicationParameters.chosenSearchResultsTableFields : ApplicationParameters.tableFields;
                                        ArrayList<TripleTableField> p2 = ApplicationParameters.chosenSearchResultSingleFields.size() > 0 ? ApplicationParameters.chosenSearchResultSingleFields : ApplicationParameters.tableFields;
                                ...
                                protected void onPostExecute(JSONObject res) {
                                    if (res == null) {
                                        ...
                                        try {
                                            saveCredentials(res.getString("access_token"), "access_token");
                                            Bookmark bm = new Bookmark(Long.parseLong(res.getJSONArray("res").getJSONObject(0).getString("ID")), value.toString(), jsonParameters, res.getString("fields_order"));
                                            ...
                                            try {
                                                ...
                                                NewSearchActivity.this.saveBookmarks();
                                                NewSearchActivity.this.setDrawerMenu();
                                            }
        ...
        public void onClick(DialogInterface dialog, int which) {
            switch (which) {
                ...
                case Dialog.BUTTON_POSITIVE:
                    ...
                    showProgress(true);
                    Intent intent = new Intent(NewSearchActivity.this, SearchResults.class);
                    try {
    ...
    public String getFieldsString() {
        String cs = "";
        if (ApplicationParameters.chosenSearchFields.size() > 0)
            cs = ApplicationParameters.chosenSearchFields.get(0).getName();
        for (int i = 1; i < ApplicationParameters.chosenSearchFields.size(); i++) {
            cs += "," + ApplicationParameters.chosenSearchFields.get(i).getName();
        }
        return cs;
    }
    ...
    public void saveChanges(SharedPreferences prefs) {
        ...
        Gson gson = new Gson();
        String json2 = gson.toJson(ApplicationParameters.chosenSearchFields);
        // String json1 = gson.toJson(ApplicationParameters.tableFields);
        // prefsEditor.putString("all", json1);
        prefsEditor.putString("filters", json2);
        prefsEditor.commit();
    }
    ...
    public String getJSONParameters() {
        ...
        while (keys.hasNext()) {
            ...
            try {
                String type = ApplicationParameters.getFieldType(key, ApplicationParameters.tableFields);
    ...
    protected void onCreate(Bundle savedInstanceState) {
        setLayoutId(R.layout.activity_new_search);
        ...
        for (int i = 0; i < ApplicationParameters.chosenSearchFields.size(); i++) {
            used.add(ApplicationParameters.chosenSearchFields.get(i));
        }
        ...
        adapter = new ChosenFieldsAdapter(this, R.layout.activity_new_search, ApplicationParameters.tableFields, lvMain, ApplicationParameters.tableFields, ApplicationParameters.chosenSearchFields, R.layout.text_edit_list_layout);
        adapter.sType = SettingsType.TABLE;
        adapter.setUsedObjects(used);
        adapterChosen = new FiltersAdapter(this, R.layout.activity_new_search, ApplicationParameters.chosenSearchFields, lvChosen);
        ...
        alertButton.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                ...
                alertAdapter = new AlertFieldsAdapter(NewSearchActivity.this, R.layout.activity_push_settings_ver2, ApplicationParameters.tableFields, lvMain, ApplicationParameters.tableFields, ApplicationParameters.chosenSearchFields);
                alertAdapter.secondListAdapter = adapterChosen;
                lvMain.setAdapter(alertAdapter);
                view2.findViewById(R.id.selectAll).setVisibility(View.GONE);
